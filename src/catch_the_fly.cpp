...
void show_text(string text, int color, double x, double y, double z) {
  ...
  // Opcija 1 je za ispis u slucaju da krtica uvlaci trenutno, tekst se ispisuje
  // crveno
  ...
  for (char i : text) glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, i);
...
static void on_timer_move_player(int value) {
  ...
  if (player_animation) glutTimerFunc(15, on_timer_move_player, TIMER_ID);
}
...
static void on_timer_move_enemyes(int value) {
  ...
  if (enemy_animation) glutTimerFunc(15, on_timer_move_enemyes, TIMER_ID);
}
...
static void on_timer_move_enemy(int value) {
  ...
  if (enemy_animation) glutTimerFunc(15, on_timer_move_enemy, value);
}
...
static void on_timer_rotate_player(int value) {
  ...
  if (rotate_animation) glutTimerFunc(15, on_timer_rotate_player, value);
}
...
static void on_timer_gate(int value) {
  ...
  if (gate_parameter > 3) gate_down = true;

  if (!gate_down) glutTimerFunc(15, on_timer_gate, value);
}
...
static void on_timer_game(int value) {
  ...
  if (!s.get_dead()) glutTimerFunc(1000, on_timer_game, value);
}
...
static void on_timer(int value) {
  ...
  if (!s.get_key()) glutTimerFunc(100, on_timer, value);
}
...
static void initialize() {
  ...
  // trazimo da nam se obezbedi 1 id i smesti u niz names kako bismo koristili
  // teksturu niz se nalazi na ovom mestu jer sam planirao da ubacim 2 teksture,
  // od toga sam za sada odustao pa i nije neophodan niz, mogla je promenljiva
